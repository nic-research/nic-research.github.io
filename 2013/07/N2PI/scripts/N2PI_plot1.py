...
filelist = glob.glob("../12-6-13/*.txt")
#filelist = glob.glob("../12-6-13/n2_scan18.txt")
print filelist
...
for datafile in filelist:
    ...
    for i in sorted(lines.keys(), key = lambda x: lines[x] ):
        if lines[i] > min(lamda) and lines[i] < max(lamda):
            print i
            axvline(lines[i],linewidth = 0.5, ls = '--', color = 'm')
...
#grid()
    ...
    plt_.show()    
    plt_.savefig(str(source + '.png'), format = 'png', dpi=180)
    close()
