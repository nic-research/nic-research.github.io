...
count = zeros(N_loop,N_loop,N_loop); %Fluorescence on cooling transition
...
for j = 1:N_loop
    for k = 1:N_loop
        for m = 1:N_loop
            ...
            rhos=steady(qo(L_s));
            count(j,k,m) = Gamma*real(sum(expect(C1dC1, rhos)));
...
xy_slice = zeros(N_loop, N_loop);
xz_slice = zeros(N_loop, N_loop);
yz_slice = zeros(N_loop, N_loop);

xy_slice = count(:,:,1);
for j = 1:N_loop
    for k = 1:N_loop
        xz_slice(j,k) = count(j,1,k);
        yz_slice(j,k) = count(5,j,k);
    end
end

figure
%plot(Bz_, count);
contourf(Bx_,By_,xy_slice)
title({'xy slice, B_z ~ 0G ', s2})
colorbar
figure
