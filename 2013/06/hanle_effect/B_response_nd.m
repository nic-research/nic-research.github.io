...
% B-field (gauss) - Bx and By *must* be zero for this version
Bx=0;
By=0;
Bz=0.05;

% Frequency spectrum
A_B = 0.01; % Amplitude of B-field modulation
F = logspace(-5, -2, 5); % Frequencies between 1E-5 and 1E-2
N_spectra = length(F); % Number of points in the B-response spectrum
T = 20*1./F; % Array of total times per spectrum (20 cycles)
N_loop = length(F);
...
s1 = ['[B_x, B_y, B_z] = [', num2str(Bx),', ', num2str(By), ...
    ', ', num2str(Bz_s(1),2),' ... ', num2str(Bz_s(end),2),'] gauss, ',...
    'dB = ', num2str(dB), ' gauss, '];
...
ylabel('\gamma /Hz')
figure
plot(Bz_s, pop_1, Bz_s, pop_2, Bz_s, pop_3);
